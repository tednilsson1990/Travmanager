...
export function veckansGenomgång(spel) {
  ...
  if (n.netto < 0) {
    ...
    stycken.push(`Ekonomin: kassan står i ${kr(spel.kassa ?? 0)} kr och veckan går ${kr(-n.netto)} kr back. I den takten räcker pengarna ${veckor} ${veckor === 1 ? "vecka" : "veckor"} — ${veckor < 5 ? "det behöver vändas nu, inte sen" : "inte akut, men håll ögonen på det"}.`);
  } else {
    stycken.push(`Ekonomin: kassan står i ${kr(spel.kassa ?? 0)} kr och veckan går ${kr(n.netto)} kr plus. Inga larm från mig.`);
  }
  ...
  if (friska.length > 0) {
    ...
    let rad = `Formen: ${bäst.namn} är stallets vassaste just nu (form ${bäst.form}).`;
  ...
  if (startklara.length > 0) {
    ...
    let rad = anmälda > 0
      ? `Starterna: ${anmälda} ${anmälda === 1 ? "anmälan är inne" : "anmälningar är inne"} — beskeden kommer onsdag.`
      : `Starterna: inget inskickat ännu.`;
    if (bästa && anmälda === 0) {
  ...
  (spel.sponsorer ?? []).forEach((a) => {
    if (a.krav && a.veckorKvar !== undefined && a.veckorKvar <= 4) {
      stycken.push(`Sponsorn: ${a.namn} utvärderar om ${a.veckorKvar} ${a.veckorKvar === 1 ? "vecka" : "veckor"} — kravet står på ${a.krav.text}, läget är ${a.krav.nu ?? 0} av ${a.krav.mål}.`);
    }
  });
  ...
  if (problem.length > 0) stycken.push(`Att hålla i huvudet: ${problem.join(" · ")}.`);
